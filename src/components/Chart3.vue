<template>
  <div class="dashed-card">
    <BasicArrowHeader title="月度排序（物业管理单位--降序）" :blockStyle="{width: '97%'}"></BasicArrowHeader>
...
export default {
  ...
  methods: {
    async refreshData () {
      let res = await fetch('https://table.cmpo1914.com/p/webapi/request/0Z5jwLh5kaep9/getSafeManagementComprehensiveList')
      res = await res.json()
      let data = res.data
      this.setOption(data)
    },
